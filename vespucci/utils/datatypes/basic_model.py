...
class DynamicTable(VespucciModel):
    ...
    def set_table_name(cls, table_name):
        ...
        '''
        cls._meta.db_table = '%s"."%s' % (current_settings.CURRENT_SCHEMA, 
                                          table_name)
        cls.name = table_name 
    ...
    def get_prep_value(self, value):
        ...
        try: return AsIs(self.range_type + '(%d,%d)' % tuple(value))
            ...
            return AsIs(self.range_type \
                        + '(%d,%d)' % tuple(self.from_db_val_to_ints(value)))
...
class BoxField(models.Field):
    ...
    def get_prep_value(self, value):
        ...
        try: return AsIs('public.make_box(%d,%d,%d,%d)' % tuple(value))
            ...
            # The value is a string from the DB
            return AsIs('public.make_box(%d,%d,%d,%d)' % tuple(
...
class CubeField(models.Field):
    ...
    def from_db_val_to_ints(self, value):
        # Comes in as '(1234),(40596)' from DB
        try: return map(lambda x: int(x.strip('(').strip(')')), 
                        value.split(','))
        except Exception: return value
    ...
    def get_prep_value(self, value):
        ...
        try: return AsIs('public.cube(%d,%d)' % tuple(value))
            ...
            # The value is a string from the DB
            return AsIs('public.cube(%d,%d)' % tuple(
                                            self.from_db_val_to_ints(value)))