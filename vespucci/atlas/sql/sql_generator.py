...
from vespucci.atlas.sql import transcripts_from_prep_functions,\
    transcripts_from_tags_functions
from vespucci.utils.database import SqlGenerator

class AtlasSqlGenerator(SqlGenerator):
    ...
    def table_main_source(self, suffix=''):
        table_name = 'atlas_transcript_source'
        return """
        CREATE TABLE "{schema_name_prefix}{suffix}"."{table_name}" (
            "id" int4 NOT NULL,
            "chromosome_id" int4 DEFAULT NULL,
            "atlas_transcript_id" int4 DEFAULT NULL,
