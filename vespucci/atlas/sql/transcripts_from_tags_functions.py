...
BEGIN 
    ...
    EXECUTE 'UPDATE atlas_{0}_{1}_prep.atlas_transcript_source_' || trans.chromosome_id || ' merged_assoc
        SET tag_count = (merged_assoc.tag_count + trans_assoc.tag_count),
            gaps = (merged_assoc.gaps + trans_assoc.gaps)
        FROM atlas_{0}_{1}_prep.atlas_transcript_source_' || trans.chromosome_id || ' trans_assoc
        WHERE merged_assoc.atlas_transcript_id = ' || trans.id || '
            AND trans_assoc.atlas_transcript_id = ' || old_id || '
    ...
    EXECUTE 'UPDATE atlas_{0}_{1}_prep.atlas_transcript_source_' || trans.chromosome_id || '
        SET atlas_transcript_id = ' || trans.id || '
        WHERE atlas_transcript_id = ' || old_id || '
        AND sequencing_run_id 
        NOT IN (SELECT sequencing_run_id 
                FROM atlas_{0}_{1}_prep.atlas_transcript_source_' || trans.chromosome_id || '
            WHERE atlas_transcript_id = ' || trans.id || ')';
    -- Delete those that remain for the removed transcript.
    EXECUTE 'DELETE FROM atlas_{0}_{1}_prep.atlas_transcript_source_' || trans.chromosome_id || '
        WHERE atlas_transcript_id = ' || old_id;
    RETURN;
END;
$$ LANGUAGE 'plpgsql';
            
CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.merge_transcripts(merged_trans atlas_{0}_{1}_prep.atlas_transcript, trans atlas_{0}_{1}_prep.atlas_transcript)
RETURNS atlas_{0}_{1}_prep.atlas_transcript AS $$
BEGIN 
...
CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.get_allowed_edge(trans atlas_{0}_{1}_prep.atlas_transcript, 
...
CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.determine_transcripts_from_sequencing_run(chr_id integer, strand integer, source_t text, max_gap integer, tag_extension integer, start_end int8range)
RETURNS SETOF atlas_{0}_{1}_prep.atlas_transcript_row AS $$
BEGIN 

    RETURN QUERY SELECT * FROM atlas_{0}_{1}_prep.determine_transcripts_from_table(chr_id, strand, source_t, max_gap, tag_extension,'', start_end);
END;
$$ LANGUAGE 'plpgsql';

CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.determine_transcripts_from_existing(chr_id integer, strand integer, max_gap integer)
RETURNS SETOF atlas_{0}_{1}_prep.atlas_transcript_row AS $$
BEGIN 
    RETURN QUERY SELECT * FROM atlas_{0}_{1}_prep.determine_transcripts_from_table(chr_id, strand, 'atlas_{0}_{1}_prep"."atlas_transcript', max_gap, 0,'transcription_', NULL);
END;
$$ LANGUAGE 'plpgsql';

CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.determine_transcripts_from_table(
...
RETURNS SETOF atlas_{0}_{1}_prep.atlas_transcript_row AS $$
 DECLARE
    ...
    row atlas_{0}_{1}_prep.atlas_transcript_row;
    last_start bigint := 0;
    last_end bigint := 0;
...
CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.save_transcripts_from_sequencing_run(
...
RETURNS VOID AS $$
 DECLARE
    rec atlas_{0}_{1}_prep.atlas_transcript_row;
    transcript atlas_{0}_{1}_prep.atlas_transcript;
    density float;
    scaling_factor text;
 ...
 BEGIN
    ...
    END IF; 
     ...
     LOOP
        FOR rec IN 
            SELECT * FROM atlas_{0}_{1}_prep.determine_transcripts_from_sequencing_run(chr_id, strand, source_t, max_gap, tag_extension, start_end)
        LOOP
            IF rec IS NOT NULL THEN
                ...
                EXECUTE 'INSERT INTO atlas_{0}_{1}_prep.atlas_transcript_' || rec.chromosome_id
                ...
                EXECUTE 'INSERT INTO atlas_{0}_{1}_prep.atlas_transcript_source_' || rec.chromosome_id || '
...
CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.save_transcripts_from_existing(chr_id integer, max_gap integer)
RETURNS VOID AS $$
 DECLARE
    rec atlas_{0}_{1}_prep.atlas_transcript_row;
    transcript atlas_{0}_{1}_prep.atlas_transcript;
 ...
 BEGIN
     ...
     LOOP
        FOR rec IN 
            SELECT * FROM atlas_{0}_{1}_prep.determine_transcripts_from_existing(chr_id, strand, max_gap)
        LOOP
            IF rec IS NOT NULL AND rec.tag_count > 1 THEN            
                ...
                LOOP
                    PERFORM atlas_{0}_{1}_prep.update_transcript_source_records(transcript, old_id);
                END LOOP;
                EXECUTE 'DELETE FROM atlas_{0}_{1}_prep.atlas_transcript_' || rec.chromosome_id
...
CREATE OR REPLACE FUNCTION atlas_{0}_{1}_prep.set_density(chr_id integer, 
...
BEGIN
    ...
    EXECUTE 'CREATE TEMP TABLE ' || table_name || '
        ...
        FROM atlas_{0}_{1}_prep.atlas_transcript_' || chr_id || ' t,
            atlas_{0}_{1}_prep.atlas_transcript_source_' || chr_id || ' source
        WHERE t.id = source.atlas_transcript_id
        ' || null_only_text || '
        GROUP BY t.id, t.transcription_start, t.transcription_end';
        
    EXECUTE 'UPDATE atlas_{0}_{1}_prep.atlas_transcript_' || chr_id || ' t
    ...
    EXECUTE 'CREATE TEMP TABLE ' || table_name || '_2
        as SELECT t.id,
                atlas_{0}_{1}_prep.get_allowed_edge(t.*,
