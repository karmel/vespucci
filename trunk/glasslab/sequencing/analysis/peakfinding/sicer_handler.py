...
class SicerHandler(object):
    def process_bed_sample(self, output_dir, file_name, sample_file, 
                           window_size=200, gap_size=400, evalue=100):
        self.remove_multiple_identical_tags(output_dir, file_name, input_file=sample_file, control=False)
        self.run_make_graph_file_by_chrom(output_dir, file_name, sample_file, window_size)
        return self.find_islands_in_pr(output_dir, file_name, sample_file, window_size, gap_size, evalue)
    ...
    def process_bed_sample_with_control(self, output_dir, file_name, sample_file, control_file, 
        ...
        self.remove_multiple_identical_tags(output_dir, file_name, input_file=sample_file, control=False)
        self.remove_multiple_identical_tags(output_dir, file_name, input_file=control_file, control=True)
        self.run_make_graph_file_by_chrom(output_dir, file_name, sample_file, window_size)
        island_file = self.find_islands_in_pr(output_dir, file_name, 
                                              sample_file, window_size, gap_size, evalue)
        differential_island_file = self.associate_tags_with_chip_and_control_w_fc_q(output_dir, file_name, 
                                                                                    control_file, sample_file, island_file)
