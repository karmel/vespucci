...
CREATE OR REPLACE FUNCTION glass_atlas_%s_%s.remove_feature_records(rec glass_atlas_%s_%s.glass_transcript, run_id integer)
RETURNS VOID AS $$
DECLARE
	feature_types text[] := ARRAY['glass_peak'];
	counter integer;
	table_type text;
BEGIN 
    FOR counter IN array_lower(feature_types,1)..array_upper(feature_types,1)
    LOOP
        table_type := feature_types[counter];
	    -- Delete associated features.
		EXECUTE 'DELETE FROM glass_atlas_%s_%s.'
			|| table_type || '_feature WHERE glass_transcript_id = ' || rec.id
...
CREATE OR REPLACE FUNCTION glass_atlas_%s_%s.insert_associated_glass_peak_features(rec glass_atlas_%s_%s.glass_transcript, chr_id integer, run_id integer)
RETURNS VOID AS $$
DECLARE
	cube text;
	large_cube text;
...
BEGIN
	...
	large_cube = 'public.cube(' || rec.transcription_start - padding || ',' || rec.transcription_end + padding || ')';
	INSERT INTO glass_atlas_%s_%s.glass_peak_feature 
	    (glass_transcript_id,
	    relationship,
		glass_peak_id,
		sequencing_run_id,
...
CREATE OR REPLACE FUNCTION glass_atlas_%s_%s.add_glass_peak_features(chr_id integer, run_id integer)
RETURNS VOID AS $$
BEGIN
	FOR rec IN
	    SELECT * FROM glass_atlas_%s_%s.glass_transcript
	    WHERE chromosome_id = chr_id
	LOOP
	    PERFORM glass_atlas_%s_%s.remove_feature_records(rec);
	    PERFORM glass_atlas_%s_%s.insert_associated_glass_peak_features(rec, chr_id, run_id);
	RETURN;
END;
$$ LANGUAGE 'plpgsql';
