...
CREATE OR REPLACE FUNCTION glass_atlas_%s_%s_staging.insert_associated_peak_features_from_run(run_id integer, chr_id integer)
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    PERFORM glass_atlas_%s_%s_staging.insert_associated_peak_features(run, chr_id);
    RETURN;
END;
$$ LANGUAGE 'plpgsql';

CREATE OR REPLACE FUNCTION glass_atlas_%s_%s_staging.insert_associated_peak_features(run glass_atlas_mm9.sequencing_run, chr_id integer)
RETURNS VOID AS $$
DECLARE
    rec record;
	padding integer := 2000;
	existing_feature glass_atlas_%s_%s_staging.peak_feature;
	distance integer;
...
BEGIN    
	...
	FOR rec in 
	   ...
	   'SELECT transcript.*, 
           ...
           (CASE WHEN transcript.start_end ~= glass_peak.start_end THEN
                glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''is equal to'')
            WHEN transcript.start_end @> glass_peak.start_end THEN
                glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''contains'')
            WHEN transcript.start_end <@ glass_peak.start_end THEN
                glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''is contained by'')
            WHEN transcript.start_end && glass_peak.start_end THEN
                glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''overlaps with'')
            ...
            WHEN (transcript.strand = 0 
                ...
                glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''is downstream of'')
            WHEN (transcript.strand = 1 
                AND public.make_box(transcription_end, 0, transcription_end + ' || padding || ', 0)
                    && glass_peak.start_end) THEN
                glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''is downstream of'')
            ELSE glass_atlas_%s_%s_staging.glass_transcript_feature_relationship(''is upstream of'')
            END) as relationship
        FROM glass_atlas_%s_%s_staging.glass_transcript transcript
    ...
    LOOP
        IF (rec.id IS NOT NULL) THEN
            ...
            existing_feature := (SELECT (feat.*)::glass_atlas_%s_%s_staging.peak_feature
                            FROM glass_atlas_%s_%s_staging.peak_feature feat
                            WHERE feat.glass_transcript_id = rec.id
                            AND feat.glass_peak_id = rec.glass_peak_id
                            AND feat.sequencing_run_id = run.id);
        	
        	IF rec.relationship = glass_atlas_%s_%s_staging.glass_transcript_feature_relationship('is downstream of')
                OR rec.relationship = glass_atlas_%s_%s_staging.glass_transcript_feature_relationship('is upstream of')
            ...
            IF existing_feature IS NULL THEN
                INSERT INTO glass_atlas_%s_%s_staging.peak_feature
            ...
            ELSE
                UPDATE glass_atlas_%s_%s_staging.peak_feature 
                    SET 
...
CREATE OR REPLACE FUNCTION glass_atlas_%s_%s_staging.update_peak_features(chr_id integer, run_requires_reload_only boolean)
RETURNS VOID AS $$
DECLARE
    run glass_atlas_mm9.sequencing_run;
BEGIN
